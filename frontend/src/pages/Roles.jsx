...
const Roles = () => {
    ...
    const handleEdit = async (item) => {
        try {
            const fullItem = await getRolById(item.id);
    ...
    const handleFormSuccess = () => {
        handleCloseForm();
        setSuccess(editingItem ? 'Rol actualizado correctamente' : 'Rol creado correctamente');
        loadItems();
    };
    ...
    const handleConfirmBulkDelete = async () => {
        try {
            await deleteRolesBulk(Array.from(selectedIds));
            setSuccess(`${selectedIds.size} rol(es) desactivado(s) correctamente`);
            setSelectedIds(new Set());
            loadItems();
        } catch (err) {
            setError(err.message);
    ...
    const handleReactivate = async (item) => {
        try {
            await reactivateRol(item.id);
            setSuccess('Rol reactivado correctamente');
            loadItems();
        } catch (err) {
            setError(err.message);
        }
            ...
            <div className="card">
                <div className="card-header">
                    <div className="header-left">
                        ...
                        <span className="selection-indicator">
                            {selectedIds.size} de {items.length} seleccionados
                        </span>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="empty-state">
                        ...
                        <h3>No hay roles {showingInactive ? 'inactivos' : ''}</h3>
                        <p>{showingInactive ? 'No hay roles desactivados' : 'Crea un nuevo rol para comenzar'}</p>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody>
                                    {items.map((item) => (
                                        <tr key={item.id} className={`${selectedIds.has(item.id) ? 'row-selected' : ''} ${!item.activo ? 'row-inactive' : ''}`}>
                                            ...
                                            {visibleColumns.usuarios && (
                                                <td>
                                                    <span className="badge badge-secondary">
                                                        {item.usuariosCount || 0} usuarios
                                                    </span>
                                                </td>
                                            )}
                                            {visibleColumns.permisos && (
                                                <td>
                                                    <span className="badge badge-info">
                                                        {item.permisos?.length || 0} permisos
                                                    </span>
                                                </td>
                                            )}
                                            <td>
