...
const ConceptosSalarialesModal = ({ onClose }) => {
    ...
    const loadConceptos = async () => {
        try {
            setLoading(true);
            const data = await getConceptosSalariales({ activo: 'true' });
            setConceptos(data);
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await updateConceptoSalarial(editingId, formData);
            } else {
                await createConceptoSalarial(formData);
            }
            ...
            <div className="modal-overlay" onClick={onClose}>
                <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '950px' }}>
                    ...
                    <div className="modal-body" style={{ padding: '2rem' }}>
                        ...
                        {/* Formulario */}
                        <form onSubmit={handleSubmit} style={{ marginBottom: '2rem', padding: '1.5rem', background: 'var(--bg-secondary)', borderRadius: '8px' }}>
